// merges a list of objects, tracking which have been added with the __self attribute
module.exports.merge = function() {
	var a, k, obj = arguments[1];

	// set self to a non-enumerable map
	if (typeof self != 'object') {
		var self = obj.__self;
		Object.defineProperty(obj, '__self', { value: {} });
		if (self) { obj.__self[self] = true; }
	}
	...
	for (var i=1; 1 < arguments.length; i++) {
		...
		// merge __self
		if (a.__self) {
			if (typeof a.__self == 'object') {
				for (k in a.__self) { obj.__self[k] = true; }
			} else {
				obj.__self[a.__self] = true;
			}
		}
...
// mixin to prune all falsey values
module.exports.consensable = function(ctx) {
	return { __condense: __condense };
};
function __condense() {
	for (var k in this) {
		if (!this[k]) {
...
var makeSafe = module.exports.makeSafe;
...
module.exports.pad0 = function(n, width, z) {
...
module.exports.renderLinkEl = function(link) {
	var attrs = [];
	for (var k in link) {
		attrs.push(k+'="'+link[k]+'"');
	}
...
module.exports.serializeLinkObject = function (link) {
	var linkParts = ['<'+makeSafe(link.href)+'>'];
	for (var attr in link) {
		if (attr == 'href') {
			continue;
		}
		if (typeof link[attr] == 'boolean') {
			linkParts.push(makeSafe(attr));
		} else {
			linkParts.push(makeSafe(attr)+'="'+makeSafe(link[attr])+'"');
		}
	}
